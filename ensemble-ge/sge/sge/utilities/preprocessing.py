from sklearn.preprocessing import Binarizer, MaxAbsScaler, MinMaxScaler, Normalizer, PolynomialFeatures, StandardScaler, RobustScaler
...
def fast_ica(args):
	...
	return FastICA(algorithm=args['algorithm'], 
...
def gaussian_random_projection(args):
	...
	return GaussianRandomProjection(eps=args['eps'], random_state=42)
...
def imputer(args):
	...
	return SimpleImputer(missing_values='NaN', strategy=args['strategy'], axis=0, copy=True)
...
def incremental_pca(args):
	...
	return IncrementalPCA( whiten=args['whiten'], copy=True)
...
def pca(args):
	...
	return PCA(whiten=args['whiten'], copy=True,
			   random_state=42)
...
def rbf_sampler(args):
	...
	return RBFSampler(gamma=args['gamma'], random_state=42)
...
def sparse_random_projection(args):
	...
	return SparseRandomProjection(density=args['density'], eps=args['eps'],
...
def truncated_svd(args):
	...
	return TruncatedSVD(algorithm=args['algorithm'], n_iter=args['n_iter'],
						tol=args['tol'], random_state=42)